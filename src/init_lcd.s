...
initLCD
			...
			LDR		R1,=GPIO_PORTA_DIR		; PA2,3,4,5,6,7 Output
			LDR 	R0, [R1]
			ORR		R0, #0xFC
			STR		R0,[R1]
			
			LDR		R1,=GPIO_PORTA_AFSEL	; AFSEL on PA2,3,5
			LDR 	R0, [R1]
			ORR		R0, #0x2C
			STR		R0,[R1]
			...
			LDR		R1,=GPIO_PORTA_PCTL 	; PA2,3,5 as SSI
			MOV32	R0, #0x00220200
			STR		R0,[R1]
			
			LDR		R1,=GPIO_PORTA_AMSEL	; no AMSEL
			LDR 	R0, [R1]
			BIC		R0, #0xFC
			STR		R0,[R1]
			...
			MOV		R5,#0x04		;TEMP
			BL		LCD_PRINT_BYTE
			;0x14 or 0x15 is good for BIAS
			MOV		R5,#0x15		;BIAS
			BL		LCD_PRINT_BYTE
			MOV		R5,#0x20		;H=0
			BL		LCD_PRINT_BYTE
			MOV		R5,#0x0C		;NORMAL DISPLAY
			BL		LCD_PRINT_BYTE
