...
void init_func(void)
{
	...
	// PE3 (AIN0) -> ADC0
	// PE2 (AIN1) -> ADC1
	GPIOE->DIR &= ~0x0C; //set pin PE3 & PE2 as an input
	GPIOE->AFSEL |= 0x0C; //enable alternate function select for PE3 & PE2
	GPIOE->DEN &= ~0x0C; //disable digital function of pin PE3 & PE2
	GPIOE->AMSEL |= 0x0C; //enable analog function of pin PE3 & PE2
